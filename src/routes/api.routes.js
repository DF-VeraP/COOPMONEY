...
router.get('/superadmin/stats', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  try {
...
// GET /api/superadmin/cooperativas
router.get('/superadmin/cooperativas', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  try {
    // Las ya registradas
...
router.post('/superadmin/solicitudes/:id/aprobar', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    
    // Obtener solicitud
    const { rows: solRows } = await client.query('SELECT * FROM solicitud_afiliacion WHERE id_solicitud = $1', [req.params.id]);
    if (solRows.length === 0) throw new Error("Solicitud no encontrada");
    const sol = solRows[0];

    // Insertar en cooperativa
    const { rows: coopRows } = await client.query(`
      INSERT INTO cooperativa (nombre_cooperativa, nit_cooperativa, direccion_cooperativa, telefono_cooperativa, correo_cooperativa, estado_cooperativa)
      VALUES ($1, $2, $3, $4, $5, 'activo') RETURNING id_cooperativa
    `, [sol.nombre_cooperativa, sol.nit_cooperativa, sol.direccion_cooperativa, sol.telefono_cooperativa, sol.correo_cooperativa]);
    
    const newCoopId = coopRows[0].id_cooperativa;

    // Insertar el admin local en usuario
    // El documento, correo y telefono se toman del representante
    await client.query(`
    ...
    `, [newCoopId, sol.nombre_representante, sol.cedula_representante, sol.correo_representante, sol.contrasena_admin, sol.telefono_representante]);

    // Eliminar solicitud
    await client.query('DELETE FROM solicitud_afiliacion WHERE id_solicitud = $1', [sol.id_solicitud]);
...
router.post('/superadmin/solicitudes/:id/rechazar', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  try {
    await db.query('DELETE FROM solicitud_afiliacion WHERE id_solicitud = $1', [req.params.id]);
...
// POST /api/superadmin/cooperativas (Crear directamente)
router.post('/superadmin/cooperativas', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  const client = await db.connect();
  try {
    ...
    // Validar duplicados de cooperativa (nit o correo)
    const coopCheck = await client.query('SELECT id_cooperativa FROM cooperativa WHERE nit_cooperativa = $1 OR correo_cooperativa = $2', [nit, correo]);
    ...
    const newCoopId = coopRes.rows[0].id_cooperativa;

    // Hash contraseña
    const salt = await bcrypt.genSalt(12);
    const hash = await bcrypt.hash(pass_rep, salt);

    // Insertar administrador local
    await client.query(`
      INSERT INTO usuario (id_cooperativa_usuario, nombre_usuario, documento_usuario, correo_usuario, contrasena_usuario, rol_usuario, estado_usuario, telefono_usuario)
      VALUES ($1, $2, $3, $4, $5, 'admin_local', 'activo', $6)
    `, [newCoopId, nombre_rep, cedula_rep, correo_rep, hash, tel_rep]);
...
// GET /api/superadmin/cooperativas/:id/detalles (Métricas Avanzadas)
router.get('/superadmin/cooperativas/:id/detalles', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Obtener métricas financieras (socios, ahorros, cartera vigente)
    ...
    // Obtener cartera vencida (suma de saldo_restante_cuota de cuotas vencidas, y podemos agregar mora si tuviéramos tabla separada, pero por ahora sumamos cuotas vencidas)
    ...
    const { rows: base } = await db.query(`SELECT nombre_cooperativa, estado_cooperativa FROM cooperativa WHERE id_cooperativa = $1`, [id]);
    
    if(base.length === 0) return res.status(404).json({error: "Cooperativa no encontrada"});
...
// GET /api/superadmin/cooperativas/:id (Detalles para Editar)
router.get('/superadmin/cooperativas/:id', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  try {
    const { id } = req.params;
    
    const { rows: coop } = await db.query(`
      SELECT c.*, 
    ...
    if (coop.length === 0) return res.status(404).json({ error: "Cooperativa no encontrada" });
...
// PUT /api/superadmin/cooperativas/:id (Editar)
router.put('/superadmin/cooperativas/:id', authenticateJWT, authorizeRoles('super_admin'), async (req, res) => {
  const client = await db.connect();
  try {
    const { id } = req.params;
...
router.post('/auth/login', async (req, res) => {
  ...
  try {
    // 1. Escenario Super Administrador (cooperativa vacía)
    if (!cooperativaId || cooperativaId.trim() === '') {
      // Buscar usuario super administrador
      const { rows } = await db.query(`
        SELECT id_usuario, nombre_usuario, contrasena_usuario, rol_usuario, estado_usuario 
        FROM usuario 
        WHERE (correo_usuario = $1 OR documento_usuario = $1) 
          AND rol_usuario = 'super_admin'
      `, [usuario]);

      if (rows.length === 0) {
        return res.status(401).json({ error: "Credenciales incorrectas o no es Super Administrador." });
      }
      ...
      // Validar contraseña
      let isValid = false;
      if (user.contrasena_usuario === "$2b$12$HASH_CAMBIAR_EN_PRIMER_LOGIN" && password === "admin") {
         // Backdoor temporal para la contraseña de prueba que mencionaste
         isValid = true;
      } else {
         // Validación real con bcrypt
         isValid = await bcrypt.compare(password, user.contrasena_usuario);
      }
    ...
    const cooperativa = coopRes.rows[0];
    ...
    // Buscar el usuario dentro de esa cooperativa
    const userRes = await db.query(`
      ...
      WHERE id_cooperativa_usuario = $1 
        AND (correo_usuario = $2 OR documento_usuario = $2)
    `, [cooperativa.id_cooperativa, usuario]);

    if (userRes.rows.length === 0) {
      return res.status(401).json({ error: "Usuario no encontrado en la cooperativa seleccionada." });
    }
    ...
    if (user.rol_usuario === 'socio') {
      ...
      `, [user.id_usuario]);
      socioId = socioRes.rows.length > 0 ? socioRes.rows[0].id_socio : null;
    }
